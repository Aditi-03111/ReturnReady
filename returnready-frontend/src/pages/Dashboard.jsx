...
>
  Refresh analysis
      ...
      <div className="grid grid-cols-3 gap-4 mb-10">
        <div className="bg-surface rounded-2xl p-4 text-center border border-stone-200">
          <p className="font-display text-3xl text-terra">{strongCount}</p>
          <p className="text-muted text-xs mt-1">Strong skills</p>
        </div>
        <div className="bg-surface rounded-2xl p-4 text-center border border-stone-200">
          <p className="font-display text-3xl text-amber-600">{decayedCount}</p>
          <p className="text-muted text-xs mt-1">Need refresh</p>
        </div>
        <div className="bg-surface rounded-2xl p-4 text-center border border-stone-200">
          <p className="font-display text-3xl text-ink">{missingCount}</p>
      ...
      <div className="space-y-5">
        ...
        <div
          ...
          className="bg-surface border border-stone-200 rounded-2xl p-6 cursor-pointer hover:border-terra transition group"
        >
          <div className="flex items-start justify-between">
            <div>
              <p className="text-terra text-xs font-semibold tracking-widest uppercase mb-1">Mirror</p>
              <h2 className="font-display text-2xl text-ink mb-2">Your Skills Constellation</h2>
              <p className="text-muted text-sm leading-relaxed">
                See where your skills stand today — what's still strong, what's faded, what's missing.
              </p>
            </div>
            <span className="text-stone-300 group-hover:text-terra text-2xl transition ml-4">→</span>
          </div>
          <div className="mt-4 flex gap-2 flex-wrap">
            {gap_analysis.strong_skills.slice(0, 3).map((s) => (
              <span key={s} className="bg-green-100 text-green-800 text-xs px-3 py-1 rounded-full">{s}</span>
            ))}
            {gap_analysis.decayed_skills.slice(0, 2).map((s) => (
              <span key={s.skill_name} className="bg-amber-100 text-amber-800 text-xs px-3 py-1 rounded-full">{s.skill_name}</span>
            ))}
          </div>
        </div>
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="font-display text-2xl text-ink mb-2">This Week's Actions</h2>
              <p className="text-muted text-sm leading-relaxed">
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="font-display text-2xl text-ink mb-2">Your Silent Peers</h2>
              <p className="text-muted text-sm leading-relaxed">
          ...
          <div className="mt-4 flex gap-2">
            {["AR", "SM", "KP"].map((initials) => (
              <div key={initials} className="w-8 h-8 rounded-full bg-terra text-sand text-xs font-semibold flex items-center justify-center">
                {initials}
              </div>
            ))}
            <div className="w-8 h-8 rounded-full bg-stone-200 text-muted text-xs flex items-center justify-center">
              +2
            </div>
          </div>
        </div>
      ...
      {/* Summary quote */}
      <div className="mt-10 bg-terra/10 border border-terra/20 rounded-2xl p-6">
