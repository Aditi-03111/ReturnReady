...
export default function Witness() {
  ...
  return (
    <div className="min-h-screen bg-sand px-6 py-10 max-w-3xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="font-display text-5xl text-ink">You're not alone</h1>
      ...
      {nopeers ? (
        <div className="bg-surface border border-stone-200 rounded-2xl p-8 text-center mb-8">
          <p className="font-display text-2xl text-ink mb-2">No peers yet</p>
        ...
        <div className="space-y-4 mb-10">
          {peers.map((peer) => (
            <div key={peer.id}
              className="bg-surface border border-stone-200 rounded-2xl p-6 hover:border-terra transition">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-full bg-terra text-sand font-semibold flex items-center justify-center text-sm flex-shrink-0">
                  {peer.initials}
                </div>
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    ...
                    <span className="bg-terra/10 text-terra text-xs px-3 py-1 rounded-full font-semibold flex-shrink-0">
  ...
  className={`mt-4 text-xs px-4 py-2 rounded-xl font-medium transition ${
    waved[peer.id]
      ...
      : "border border-terra text-terra hover:bg-terra hover:text-sand"
  }`}
>
  {waved[peer.id] ? "✦ Wave sent — they'll feel it" : "Send a silent wave →"}
      ...
      <div className="bg-terra/10 border border-terra/20 rounded-2xl p-6 mb-8">
